...
def profile_model(model, name="Model", runs=5, log_dir="profiler_logs"):
    ...
    with profile(
        ...
        record_shapes=True,
        with_stack=True,
        on_trace_ready=tensorboard_trace_handler(log_dir + f"/{name.replace(' ', '_')}"),
    ) as prof:
        with torch.no_grad():
            for _ in range(runs):
    ...
    # Avg time per run
    total_time = sum([evt.cpu_time_total for evt in prof.key_averages()])/1e6 / runs
    print(f"[{name}] Avg forward time: {total_time*1000:.2f} ms, Peak memory: {mem:.2f} MB")

    # Print per-layer FLOPs estimates
    print(f"\nPer-layer FFN FLOPs (GFLOPs) for {name}:")
    for i in range(n_layers):
        dense, sparse = estimate_ffn_flops(hidden_size, hidden_size*ffn_expansion, seq_len, batch_size, sparsity=0.5 if "Sparse" in name else 0.0)
        print(f"Layer {i+1}: Dense={dense/1e9:.2f} GFLOPs, Sparse={sparse/1e9:.2f} GFLOPs, Reduction={(1 - sparse/dense)*100:.1f}%")
